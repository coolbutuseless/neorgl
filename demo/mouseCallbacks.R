...
mouseTrackball <- function(button = 1, dev = rgl.cur()) {
  ...
  for (i in dev) {
    rgl.set(i, TRUE)
    rgl.setMouseCallbacks(button,
      begin = trackballBegin, update = trackballUpdate, end = NULL,
      dev = dev
    )
  }
  rgl.set(cur, TRUE)
...
mouseZoom <- function(button = 1, dev = rgl.cur()) {
  mouseInterp(button,
    dev = dev, fn = par3dinterp(times = c(-4, 4) / 4, zoom = c(10^(-4), 10^4), method = "linear"),
...
mouseFOV <- function(button = 1, dev = rgl.cur()) {
  mouseInterp(button,
    dev = dev, fn = par3dinterp(times = c(1, 179) / 180, FOV = c(1, 179), method = "linear"),
