...
hist3d <- function(x, y = NULL, nclass = "auto", alpha = 1, col = "#ff0000", scale = 10) {
  ...
  n <- length(x)
  if (nclass == "auto") {
    nclass <- ceiling(sqrt(nclass.Sturges(x)))
  }
...
zl <- c(
  0, z, 0, z, z + of, z + of, -of, -of, 0, 0, z, z, 0, z, 0, z, rep(0, 4), rep(z, 4), rep(-of, 4),
  rep(z + of, 4), z + of, z + of, -of, -of, rep(c(0, z), 4), 0, 0, z, z
)
yl <- c(
  yl2, y[2] + of, y[2] + of, rep(c(y[1], y[2]), 4), y[1], y[1], y[2], y[2], yl2,
