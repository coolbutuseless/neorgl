...
lollipop3d <- function(data.x, data.y, data.z, surf.fun, surf.n = 50,
  ...
  bg3d(color = c(bg, fg))
  if (!missing(surf.fun)) {
    surface3d(surf.x, surf.y, surf.z,
      alpha = alpha.surf,
  ...
  ## draw lollipops
  apply(
    cbind(data.x, data.y, data.z, z.interc), 1,
    function(X) {
      lines3d(
        x = rep(X[1], 2),
        y = rep(X[2], 2),
  ...
  if (axes == "box") {
    ...
    bbox <- par3d("bbox")
    axis3d(
      edge = "x", at = x.ticks, labels = x.ticklabs,
      col = col.axes, arrow = axis.arrow
    )
    axis3d(
      edge = "y", at = y.ticks, labels = y.ticklabs,
      col = col.axes, arrow = axis.arrow
    )
    axis3d(
      edge = "z", at = z.ticks, labels = z.ticklabs,
      col = col.axes, arrow = axis.arrow
    )
    box3d(col = col.axes)
  }
