...
subsetSlider <- function(subsets, labels = names(subsets),
  ...
  propertySlider(subsetSetter(subsets,
    fullset = fullset,
...
propertySlider <- function(setter = propertySetter,
  ...
  if (is.null(outputid) || is.null(labels)) {
    outputfield <- setoutput <- ""
  } else {
  ...
  for (i in seq_along(setters)) {
    ...
    result <- c(
      ...
      if (!inherits(setter, "indexedSetter")) {
        ...
        subst(
          "       %settername%(value, %index%);", settername,
          index = index - 1
        )
      }
    )
...
propertySetter <- function(values = NULL, entries, properties, objids, prefixes = "",
  ...
  if (!ncol) {
    return(structure("function(value) {}",
      param = param,
      prefixes = prefixes,
  ...
  for (j in seq_along(entries)) {
    ...
    result <- c(
      ...
      if (newprefix != prefix || newprop != property || newid != objid) {
        subst(
...
vertexSetter <- function(values = NULL, vertices = 1, attributes, objid, prefix = "",
  ...
  direct <- is.null(values)
  ncol <- max(
    if (is.matrix(values)) ncol(values),
  ...
  if (ofs < 0)
    alert("Attribute %attribute% not found in object %objid%");',
        prefix, objid,
        attribofs = attribofs[attributes[j]],
        attribute = attributes[j]
      ),

      if (attribofsofs[attributes[j]] > 0) {
        subst(
          "  ofs = ofs + %attribofsofs%;",
          attribofsofs = attribofsofs[attributes[j]]
        )
      },
...
par3dinterpSetter <- function(fn, from, to, steps, subscene = NULL,
  ...
  propertySetter(
    values = values[, keep], entries = entries[keep],
...
matrixSetter <- function(fns, from, to, steps, subscene = currentSubscene3d(), matrix = "userMatrix",
  ...
  result <- c(
   ...
   var newmatrix = new CanvasMatrix4();",
    paste0(subst(
      "   newmatrix.multLeft(%prefix%rgl.%propname%[", prefix, propname
    ), 0:(n - 1), "]);"),
    subst(
 ...
 %prefix%rgl.%propname% = new Array();", prefix, subscene, propname, matrix
    ...
    paste0(subst(
      " %prefix%rgl.%propname%[", prefix, propname
    ), 0:(n - 1), "] = new CanvasMatrix4();")
  )
  structure(paste(result, collapse = "\n"),
...
ageSetter <- function(births, ages, colors = NULL, alpha = NULL,
  ...
  if ("colors" %in% attribs) {
    result <- c(result, subst(
      "    var colors = [%values%];",
      values = formatVec(colors[rows, ])
    ))
  }
  if ("alpha" %in% attribs) {
    result <- c(result, subst(
      "    var alpha = [%values%];",
      values = formatVec(alpha[rows])
    ))
  }
  if ("radii" %in% attribs) {
    result <- c(result, subst(
      "    var radii = [%values%];",
      values = formatVec(radii[rows])
    ))
  }
  if ("vertices" %in% attribs) {
    ...
    result <- c(result, subst(
      "    var vertices = [%values%];",
      values = formatVec(vertices[rows, ])
    ))
  }
  ...
  if ("origins" %in% attribs) {
    ...
    result <- c(result, subst(
      "    var origins = [%values%];",
      values = formatVec(origins[rows, ])
    ))
  }
  if ("texcoords" %in% attribs) {
    stopifnot(ncol(texcoords) == 2)
    result <- c(result, subst(
      "    var texcoords = [%values%];",
      values = formatVec(texcoords[rows, ])
    ))
  }
  result <- c(
    result,
  ...
  for (j in seq_len(nobjs)) {
    ...
    for (a in attribs) {
      ofs <- c(
        colors = "cofs", alpha = "cofs", radii = "radofs",
