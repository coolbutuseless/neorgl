...
#' 
#' This function writes PLY files.  This is a simple file format that is
#' commonly used in 3D printing.  It does not represent text, only edges and
#' polygons.  The \code{writePLY} function does the necessary conversions.
#' 
...
#' chosen because Blender (\url{http://www.blender.org}) does not import lines,
#' only polygons.  If you are exporting to other software you may want to
#' change them.
#' 
#' Since the PLY format only allows one object per file, all RGL objects are
...
#' @param format Which output format.  Defaults to little-endian binary.
...
#' @param pointsAsEdges,linesAsEdges Whether to convert points and lines to
#' \dQuote{Edge} records in the PLY output.
#' @param withColors Whether to output vertex color information.
...
#' \code{\link{rglwidget}}, \code{\link{writeASY}}, \code{\link{writeOBJ}} and
#' \code{\link{writeSTL}} write the scene to a file in various other formats.
#' @references The file format was found on \url{http://www.mathworks.com} on
#' November 10, 2012 at a URL that no longer exists; currently the format is
#' described at
#' \url{https://www.mathworks.com/help/vision/ug/the-ply-format.html}.
#' @keywords graphics
#' @examples
#' 
