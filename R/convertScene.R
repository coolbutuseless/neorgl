...
convertScene <- function(x = scene3d(minimal), width = NULL, height = NULL, reuse = NULL,
  ...
  getFlags <- function(id) {
    ...
    mat <- getMaterial(id)
    result["is_lit"] <- mat$lit && type %in% c(
      "triangles", "quads", "surface", "planes",
      "spheres", "sprites"
    )

    result["is_smooth"] <- mat$smooth && type %in% c(
      "triangles", "quads", "surface", "planes",
      "spheres"
    )
    ...
    result["depth_sort"] <- depth_sort <- is_transparent && type %in% c(
      "triangles", "quads", "surface",
  ...
  knowntypes <- c(
    "points", "linestrip", "lines", "triangles", "quads",
  ...
  if (is.null(reuse) || isTRUE(reuse)) {
    reuseDF <- data.frame(
      id = numeric(), elementId = character(), texture = character(),
  ...
  if (any(simple)) {
    reuseDF <- rbind(reuseDF, data.frame(
      id = ids[simple],
      elementId = elementId,
  ...
  if (length(sphereId)) {
    ...
    x <- tmesh3d(
      vertices = rbind(qx, qy, qz, 1),
      texcoords = cbind(
        rep(fx, each = sections + 1),
        rep(fy, segments + 1)
      ),
      indices = cbind(
        rbind(
          inds, inds + sections + 1,
          inds + sections + 2
        ),
        rbind(
          inds, inds + sections + 2,
          inds + 1
        )
      )
    )
