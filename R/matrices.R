...
asHomogeneous <- function(x) {
  if (is.matrix(x) && dim(x)[2] == 3) {
    return(cbind(x, 1))
  } else if (length(x) == 3) {
    return(c(x, 1))
  } else {
    stop("'x' is not row vector(s)")
  }
}
...
translate3d.default <- function(obj, x, y, z, ...) {
  ...
  if (length(obj) == 3 || (is.matrix(obj) && dim(obj)[2] == 3)) {
    ...
    stop("Unsupported object for translation")
  }
}
...
scale3d.default <- function(obj, x, y, z, ...) {
  ...
  if (length(obj) == 3 || (is.matrix(obj) && dim(obj)[2] == 3)) {
    ...
    stop("Unsupported object for scaling")
  }
}
...
rotate3d.default <- function(obj, angle, x, y, z, matrix, ...) {
  if (length(obj) == 3 || (is.matrix(obj) && dim(obj)[2] == 3)) {
    ...
    stop("Unsupported object for rotation")
  }
}
